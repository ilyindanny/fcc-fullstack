...
const LOG_FILE = path.join(__dirname, 'processed.log');
...
// Основная функция
async function updateMarkdownHeaders(startPath) {
    ...
    for (const item of items) {
      ...
      if (stats.isDirectory()) {
        if (item === 'note') {
          ...
          await updateMarkdownHeaders(itemPath); // Рекурсивно
        }
      }
    }
  } catch (err) {
...
async function processNoteDirectory(notePath) {
  try {
    ...
    if (markdownFile) {
      ...
      const updatedContent = content.replace(/^#+/gm, match => '##' + match);
      await fs.writeFile(filePath, updatedContent, 'utf-8');

      console.log(`[Обновлено] Заголовки в: ${filePath}`);
      return true;
    } else {
...
console.log('Начинаю поиск и обновление заголовков...');
updateMarkdownHeaders(startDirectory).then(() => {
  console.log('Обновление заголовков завершено.');
}).catch(err => {
